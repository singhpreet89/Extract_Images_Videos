                ...
                try:
                    ...
                    if (format == '*.jpg' or format == '*.jpeg') and (os.path.getsize(file) / 1000) >= self.MINIMUM_SIZE_IMAGE:
                        ...
                        if len(Path(file).suffixes) == 1:
                            file_name = Path(file).stem.split('.')[0]
                        elif len(Path(file).suffixes) > 1:
                            file_name = reduce(lambda extension, each_extension : extension + '.' + each_extension, Path(file).stem.split('.'))
                        Image.open(file).convert("RGB").save(os.path.normpath(os.path.join(directory, 'images', file_name + ".jpg")), "jpeg")
                    elif format == '*.mp4':
